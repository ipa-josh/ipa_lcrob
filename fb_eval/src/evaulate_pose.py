...
import rosbag, os, math, rospy, sys
...
topic_ms="/karto_pose" #"/ExperienceMap/RobotPose"
...
for fn_bag in sys.argv[2:]:
	...
	for ts in sorted(poses_gt):
		...
		res.append([transformations.vector_norm(transformations.euler_from_matrix(D, 'rxyz')), transformations.vector_norm(D[:3, 3]),ts.to_sec()])
